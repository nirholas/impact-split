...
/**
 * @title DeployImpactSplit
 * @notice Deploys ImpactSplit deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployImpactSplit is Script {
    uint160 internal constant FLAGS = uint160(Hooks.AFTER_INITIALIZE_FLAG | Hooks.BEFORE_SWAP_FLAG | Hooks.AFTER_SWAP_FLAG | Hooks.AFTER_SWAP_RETURNS_DELTA_FLAG);
    ...
    function run() external {
        ...
        bytes memory creationCode = type(ImpactSplitHook).creationCode;
        ...
        console2.log("ImpactSplit ", predicted);
        ...
        vm.startBroadcast();
        ImpactSplitHook hook = new ImpactSplitHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("ImpactSplit")),
