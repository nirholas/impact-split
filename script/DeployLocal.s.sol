...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(ImpactSplitHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new ImpactSplitHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, ImpactSplitHook.Config({maxImpactFee: 50_000, halfPointTicks: 200, windowSeconds: 1 hours, minTicks: 5}));
