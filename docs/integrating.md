# Integrating ImpactSplit

## From Solidity

```solidity
import {ImpactSplitHook} from "impact-split/src/hooks/ImpactSplitHook.sol";
...
hook.configure(key, ImpactSplitHook.Config({ /* see the README */ }));
...
const hook = getHook("impact-split");
const address = hookAddress("impact-split", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "ImpactSplit"
...
More at https://impact-split.pages.dev.
